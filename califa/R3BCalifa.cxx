...
// -----       		Modified by 1/10/18 by E.Galiana/H. Alvarez    -----
// -------------------------------------------------------------------------
////////////////////////////////////////////////////////////////////////////
...
R3BCalifa::R3BCalifa(const TString& geoFile, const TGeoCombiTrans& combi)
  ...
  LOG(INFO) << "R3BCalifa::R3BCalifa default fRegisterMode = " << fRegisterMode 
	    << FairLogger::endl;
  LOG(INFO) << "where 0 means only R3BCalifaCrystalCalData, 1 only R3BCalifaPoint and 2 both." 
	    << FairLogger::endl;
...
Bool_t R3BCalifa::ProcessHits(FairVolume* vol) {
  ...
  if (fCrystal == NULL) {
    ...
    // -> Something went wrong, but the user should already have been informed
    LOG(INFO) << "R3BCalifa: ProcessHits. fCrystal null by unknown reason. Unprocessed hit." 
	      << FairLogger::endl;
    return kFALSE;
  }
  
  if (fVerboseLevel > 1)
    LOG(INFO) << "R3BCalifa: Processing Points/CalData in Alveolus Nb " << fCrystal->volIdAlv << ", copy Nb "
  ...
  if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
    ...
    // Adding a crystalCal support
    if(fRegisterMode==0 || fRegisterMode==2){
      Int_t nCrystalCals = fCalifaCrystalCalCollection->GetEntriesFast();
      Bool_t existHit = 0;
      
      if (nCrystalCals == 0) {
	AddCrystalCal(fCrystal->crystalId,
		      NUSmearing(fELoss),
		      fNf,
		      fNs,
		      fTime,
	...
	for (Int_t i = 0; i < nCrystalCals; i++) {
	  if (((R3BCalifaCrystalCalData*)(fCalifaCrystalCalCollection->At(i)))->GetCrystalId() == fCrystal->crystalId) {
	    ((R3BCalifaCrystalCalData*)(fCalifaCrystalCalCollection->At(i)))->AddMoreEnergy(NUSmearing(fELoss));
	    ((R3BCalifaCrystalCalData*)(fCalifaCrystalCalCollection->At(i)))->AddMoreNf(fNf);
	    ((R3BCalifaCrystalCalData*)(fCalifaCrystalCalCollection->At(i)))->AddMoreNs(fNs);
	    if (((R3BCalifaCrystalCalData*)(fCalifaCrystalCalCollection->At(i)))->GetTime() > fTime) {
	      ((R3BCalifaCrystalCalData*)(fCalifaCrystalCalCollection->At(i)))->SetTime(fTime);
	    }
	    existHit = 1; // to avoid the creation of a new CrystalCal
	    break;
	  }
	}
	if (!existHit)
...
void R3BCalifa::Register()
{
  if(fRegisterMode>0){
    FairRootManager::Instance()->Register("CrystalPoint", GetName(),
					  fCalifaCollection, kTRUE);
  }
  if(fRegisterMode==0 || fRegisterMode==2){
    FairRootManager::Instance()->Register("CalifaCrystalCalData", GetName(), 
					  fCalifaCrystalCalCollection, kTRUE);
  }
}
// ----------------------------------------------------------------------------
...
{
  // HAPOL TODO -- DO I NEED TO RETURN A fCalifaCrystalCalColletion????
  // HAPOL TODO -- SHOULD I CHANGE THE ORDER TO HAVE fCalifaCrystalCalCollection BY DEFAULT????
  if (iColl == 0)
    {
    ...
    {
      return fCalifaCrystalCalCollection;
    }
  else
    return NULL;
}
...
void R3BCalifa::Print(Option_t* option) const
{
  if(fRegisterMode>0){  
    ...
    LOG(INFO) << "R3BCalifa: " << nPoints << " points registered in this event" << FairLogger::endl;
  }
  if(fRegisterMode==0 || fRegisterMode==2){
    Int_t nCrystalCals = fCalifaCrystalCalCollection->GetEntriesFast();
    LOG(INFO) << "R3BCalifa: " << nCrystalCals << " crystalCal objects registered in this event." << FairLogger::endl;
  }
}
// ----------------------------------------------------------------------------
...
R3BCalifaCrystalCalData* R3BCalifa::AddCrystalCal(Int_t ident,
						  Double_t energy,
						  Double_t Nf,
						  Double_t Ns,
						  Double_t time,
						  Double_t tot_energy)
{
  TClonesArray& clref = *fCalifaCrystalCalCollection;
  Int_t size = clref.GetEntriesFast();
  if (fVerboseLevel > 1)
    {
      LOG(INFO) << "-I- R3BCalifa: Adding CrystalCalData in detector with unique identifier " << ident 
		<< " depositing " << energy * 1e06 << " keV" << FairLogger::endl;
      LOG(INFO) << " -I- Other params: Nf " << Nf << ", Ns: " << Ns << ", time : " << time
		<< " tot_energy: " << tot_energy << FairLogger::endl;
    }
  return new (clref[size]) R3BCalifaCrystalCalData(ident, energy, Nf, Ns, time, 0);					   
}
...
// -----  Public method SetRegisterPoint  ----------------------------------
void R3BCalifa::SetRegisterPoint(Int_t value)
{
  fRegisterMode = value;
  LOG(INFO) << "R3BCalifa::SetRegisterPoint modifies fRegisterMode = " << fRegisterMode << FairLogger::endl;
  LOG(INFO) << "where 0 means only R3BCalifaCrystalCalData, 1 only R3BCalifaPoint and 2 both." << FairLogger::endl;
  return;
}
